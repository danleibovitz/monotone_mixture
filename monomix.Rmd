...
Next, we return to the motivating example, building four step models: with and without an intercept, and alternately with `Year` or `GDP` as the monotone covariate. Each model is in fact a series of 21 mixture models, 3 for each of $k = 1,...,7$. For each series, we plot the AIC and BIC per $k$, the rootogram of the model $k$ with the lowest AIC, and the monotone fits for the model $k$ with the lowest AIC.

&nbsp;  
&nbsp;  
```{r le_mod9, eval=T, results=F}
# Life expectancy models

m9 <- stepFlexmix(LifeExpectancy ~ .-1-Country.Name|Country.Name, data = le, k = 1:5, model = mono_reg(mon_inc_names = "Year")) # step model | no intercept | grouped | monotone year

```

```{r le_mod10, eval=T, results=F}

m10 <- stepFlexmix(LifeExpectancy ~ .-Country.Name|Country.Name, data = le, k = 1:5, model = mono_reg(mon_inc_names = "Year")) # step model | with intercept | grouped | monotone year

```

```{r le_mod13, eval=T, results=F}

m13 <- stepFlexmix(LifeExpectancy ~ .-1-Country.Name|Country.Name, data = le, k = 1:5, model = mono_reg(mon_inc_names = "GDP")) # step model | no intercept | grouped | monotone GDP

```

```{r le_mod14, eval=T, results=F}

m14 <- stepFlexmix(LifeExpectancy ~ .-Country.Name|Country.Name, data = le, k = 1:5, model = mono_reg(mon_inc_names = "GDP")) # step model | with intercept | grouped | monotone GDP
...
gg <- gg + theme_minimal() +
  ...
  ggtitle("Map of Country Clusters in m9") 
...
gg <- gg + theme_minimal() +
  theme(legend.direction = "horizontal") +
  ggtitle("Map of Country Clusters in m10")
...
gg <- gg + theme_minimal() + 
  theme(legend.direction = "horizontal") + 
  ggtitle("Map of Country Clusters in m13")
...
gg <- gg + theme_minimal() +
  theme(legend.direction = "horizontal") +
  ggtitle("Map of Country Clusters in m14")
